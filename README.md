...
icons/                   app icons + icon.svg source, from build/icons.mjs
...
A tacho swung past the redline. `build/icons.mjs` holds every candidate as SVG
and rasterises with Chromium, so nothing is ever upscaled.

```bash
node build/icons.mjs --sheet          # render all candidates + a contact sheet
node build/icons.mjs --pick redline   # write icons/ from one of them
```

The files are **full-bleed squares**: iOS applies its own squircle mask, and
rounding the corners in the source as well leaves the art visibly inset inside
the system's corner. The mark also stays inside the centre 80% circle, which is
what Android crops an adaptive icon to.
